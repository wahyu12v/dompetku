...
function SettingRow({ icon, color = 'blue', label, sub, right, onClick, first = false, last = false, danger = false }) {
  ...
  return (
    <div onClick={onClick}
      onMouseEnter={e => { if (onClick) e.currentTarget.style.background = 'var(--bg3)'; }}
      onMouseLeave={e => { if (onClick) e.currentTarget.style.background = 'transparent'; }}
      style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 18px', borderBottom: last ? 'none' : '1px solid var(--border)', background: 'transparent', cursor: onClick ? 'pointer' : 'default', transition: 'background 0.12s', borderTopLeftRadius: first ? 15 : 0, borderTopRightRadius: first ? 15 : 0, borderBottomLeftRadius: last ? 15 : 0, borderBottomRightRadius: last ? 15 : 0 }}>
      <div style={{ width: 34, height: 34, borderRadius: 9, background: danger ? '#fee2e2' : c.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', flexShrink: 0 }}>{icon}</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: '0.9rem', color: danger ? 'var(--red)' : 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</div>
        {sub && <div style={{ fontSize: '0.72rem', color: 'var(--text3)', marginTop: 1 }}>{sub}</div>}
      </div>
...
function PasswordInput({ value, onChange, placeholder }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <input type={show ? 'text' : 'password'} className="form-input" placeholder={placeholder} value={value} onChange={onChange} style={{ paddingRight: 42 }} />
      <button type="button" onClick={() => setShow(v => !v)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', fontSize: '1rem', lineHeight: 1, padding: 4 }}>{show ? '🙈' : '👁️'}</button>
    </div>
  );
}
...
function Panel({ open, onClose, title, children }) {
  ...
  if (!open) return null;
  return createPortal(
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 9999, background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, boxSizing: 'border-box', animation: 'fadeOverlay 0.18s ease' }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ position: 'relative', background: 'var(--card)', borderRadius: 20, border: '1px solid var(--border2)', width: '100%', maxWidth: 500, maxHeight: 'calc(100vh - 60px)', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.28)', animation: 'scaleIn 0.22s cubic-bezier(0.34,1.56,0.64,1)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 22px 16px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--card)', zIndex: 1, borderRadius: '20px 20px 0 0' }}>
          <div style={{ fontWeight: 800, fontSize: '1.05rem' }}>{title}</div>
          <button onClick={onClose} style={{ background: 'var(--bg3)', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', color: 'var(--text3)', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--red-bg)'}
...
function PanelPassword({ changePassword, onClose }) {
  ...
  return (
    <>
      {msg.text && <div className={`alert ${msg.type === 'ok' ? 'success' : 'error'} mb-3`}>{msg.text}</div>}
      <div className="form-group" style={{ marginBottom: 12 }}><label className="form-label">Password Lama</label><PasswordInput value={form.lama} onChange={set('lama')} placeholder="Password saat ini" /></div>
      <div className="form-group" style={{ marginBottom: 12 }}><label className="form-label">Password Baru</label><PasswordInput value={form.baru} onChange={set('baru')} placeholder="Minimal 4 karakter" /></div>
      <div className="form-group" style={{ marginBottom: 20 }}><label className="form-label">Konfirmasi Password Baru</label><PasswordInput value={form.konfirmasi} onChange={set('konfirmasi')} placeholder="Ulangi password baru" /></div>
      <button className="btn btn-primary w-full" onClick={save} style={{ borderRadius: 12, padding: '13px' }}>Ganti Password</button>
    </>
  );
}
...
function PanelData({ data }) {
  const doExport = (type) => {
    const exportData = { transaksi: data.transaksi, tagihan: data.tagihan, piutang: data.piutang, hutang: data.hutang, aset: data.aset, wifiIsp: data.wifiIsp, saldoAwal: data.saldoAwal, budget: data.budget, kategori: data.kategori, exportedAt: new Date().toISOString(), version: '2.0' };
    let content, filename, mime;
    if (type === 'json') { content = JSON.stringify(exportData, null, 2); filename = `dompetku_${new Date().toISOString().slice(0,10)}.json`; mime = 'application/json'; }
    else { const rows = [['Tanggal','Sumber','Pemasukan','Tujuan','Pengeluaran','MetodeBayar','Ket'], ...data.transaksi.map(t => [t.tanggal, t.sumber, t.pemasukan, t.tujuan, t.pengeluaran, t.metodeBayar||'', t.ket||''])]; content = rows.map(r => r.map(v => '"' + v + '"').join(',')).join('\n'); filename = `transaksi_${new Date().toISOString().slice(0,10)}.csv`; mime = 'text/csv'; }
    const a = Object.assign(document.createElement('a'), { href: URL.createObjectURL(new Blob([content], { type: mime })), download: filename }); a.click();
  };
  const stats = [
    { label: 'Transaksi', v: data.transaksi?.length||0, color:'var(--blue)',   bg:'var(--blue-bg)',   icon:'⇄' },
    { label: 'Tagihan',   v: data.tagihan?.length||0,   color:'var(--orange)', bg:'var(--orange-bg)', icon:'🧾' },
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {[
          ...
          <button key={btn.type} onClick={() => doExport(btn.type)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', background: 'var(--bg3)', border: '1.5px solid var(--border)', borderRadius: 12, cursor: 'pointer', textAlign: 'left', width: '100%' }}>
            <div style={{ width:36, height:36, borderRadius:9, background:'var(--card)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.1rem', flexShrink:0, boxShadow:'var(--shadow)' }}>{btn.icon}</div>
            <div><div style={{ fontWeight: 700, fontSize: '0.87rem', color: 'var(--text)' }}>{btn.label}</div><div style={{ fontSize: '0.7rem', color: 'var(--text3)', marginTop: 1 }}>{btn.sub}</div></div>
          </button>
        ))}
      </div>
    </div>
...
// ── Panel: Kategori Bebas (sumber & tujuan) ───────────────
function PanelKategori({ title, items, onAdd, onDelete, onEdit }) {
  const [newVal, setNewVal] = useState('');
  const [editIdx, setEditIdx] = useState(-1);
  const [editVal, setEditVal] = useState('');
  const doAdd = () => { const v = newVal.trim(); if (!v || items.includes(v)) return; onAdd(v); setNewVal(''); };
    ...
    <div>
      ...
      {items.length === 0
        ...
        : (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {items.map((item, idx) => (
              <div key={`${idx}-${item}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: editIdx === idx ? '4px 6px' : '6px 14px', borderRadius: 20, background: editIdx === idx ? 'var(--blue-bg)' : 'var(--bg3)', border: `1.5px solid ${editIdx === idx ? 'var(--accent2)' : 'var(--border)'}`, fontSize: '0.84rem' }}>
                {editIdx === idx ? (
                  <>
                    <input className="form-input" style={{ height: 26, padding: '2px 8px', fontSize: '0.84rem', minWidth: 80, maxWidth: 140, borderRadius: 6 }}
                      value={editVal} onChange={e => setEditVal(e.target.value)}
...
// ── Panel: Kategori Terkunci (tagihan & budget) ───────────
// defaultItems = tidak bisa dihapus/edit, userItems = bisa
function PanelKategoriLocked({ title, defaultItems, userItems, onAdd, onDelete, onEdit }) {
  const [newVal, setNewVal] = useState('');
  const [editIdx, setEditIdx] = useState(-1);
  const [editVal, setEditVal] = useState('');
  const allItems = [...defaultItems, ...userItems];
  const doAdd = () => { const v = newVal.trim(); if (!v || allItems.includes(v)) return; onAdd(v); setNewVal(''); };
  const startEdit = i => { setEditIdx(i); setEditVal(userItems[i]); };
  const cancelEdit = () => { setEditIdx(-1); setEditVal(''); };
  const saveEdit = () => { const v = editVal.trim(); if (!v) return cancelEdit(); onEdit(editIdx, v); setEditIdx(-1); setEditVal(''); };
  ...
  return (
    <div>
      {/* Input tambah baru */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <input className="form-input" placeholder={`Tambah kategori ${title.toLowerCase()} baru...`} value={newVal}
          onChange={e => setNewVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && doAdd()} style={{ flex: 1 }} />
        <button className="btn btn-primary" onClick={doAdd} disabled={!newVal.trim()} style={{ borderRadius: 10, flexShrink: 0 }}>+ Tambah</button>
      </div>

      {/* Kategori bawaan (terkunci) */}
      {defaultItems.length > 0 && (
        <>
          <div style={{ fontSize: '0.68rem', fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 8 }}>
            🔒 Bawaan (tidak bisa dihapus)
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
            {defaultItems.map((item, idx) => (
              <div key={`default-${idx}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 20, background: 'var(--bg3)', border: '1.5px solid var(--border)', fontSize: '0.84rem', opacity: 0.7 }}>
                <span style={{ fontWeight: 600 }}>{item}</span>
      ...
      {/* Kategori tambahan user (bisa edit/hapus) */}
      {userItems.length > 0 && (
        <>
          <div style={{ fontSize: '0.68rem', fontWeight: 800, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 8 }}>
            ✏️ Kategori Kamu
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {userItems.map((item, idx) => (
              <div key={`user-${idx}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: editIdx === idx ? '4px 6px' : '6px 14px', borderRadius: 20, background: editIdx === idx ? 'var(--blue-bg)' : 'var(--green-bg)', border: `1.5px solid ${editIdx === idx ? 'var(--accent2)' : '#86efac'}`, fontSize: '0.84rem' }}>
                {editIdx === idx ? (
                  <>
                    <input className="form-input" style={{ height: 26, padding: '2px 8px', fontSize: '0.84rem', minWidth: 80, maxWidth: 140, borderRadius: 6 }}
                      value={editVal} onChange={e => setEditVal(e.target.value)}
      ...
      {defaultItems.length === 0 && userItems.length === 0 && (
        <div style={{ textAlign: 'center', padding: '24px', color: 'var(--text3)', fontSize: '0.85rem', background: 'var(--bg3)', borderRadius: 12 }}>Belum ada kategori</div>
      )}
    </div>
  );
...
export default function PengaturanPage({ data, user, changePassword, updateName, resetData }) {
  ...
  const sumber     = useMemo(() => kategori?.sumber  || [], [kategori]);
  const tujuan     = useMemo(() => kategori?.tujuan  || [], [kategori]);
  // Untuk tagihan & budget: pisahkan bawaan vs user
  const katTagihanUser = useMemo(() => kategori?.tagihan || [], [kategori]);
  const katBudgetUser  = useMemo(() => kategori?.budget  || [], [kategori]);
  const { confirm: showConfirm, ConfirmUI } = useConfirm();
  ...
  // Tagihan: hanya simpan user items, bawaan dari TAGIHAN_OPTIONS
  const addTg = useCallback(v   => setKategori({...kategori, tagihan:[...katTagihanUser,v]}),                              [kategori,katTagihanUser,setKategori]);
  const delTg = useCallback(idx => setKategori({...kategori, tagihan:katTagihanUser.filter((_,i)=>i!==idx)}),             [kategori,katTagihanUser,setKategori]);
  const edtTg = useCallback((idx,v)=>{const a=[...katTagihanUser];a[idx]=v;setKategori({...kategori,tagihan:a});},[kategori,katTagihanUser,setKategori]);

  // Budget: bawaan = TAGIHAN_OPTIONS, user items disimpan terpisah
  const addBg = useCallback(v   => setKategori({...kategori, budget:[...katBudgetUser,v]}),                               [kategori,katBudgetUser,setKategori]);
  const delBg = useCallback(idx => setKategori({...kategori, budget:katBudgetUser.filter((_,i)=>i!==idx)}),              [kategori,katBudgetUser,setKategori]);
  const edtBg = useCallback((idx,v)=>{const a=[...katBudgetUser];a[idx]=v;setKategori({...kategori,budget:a});},[kategori,katBudgetUser,setKategori]);
  ...
  const totalRecords = (data.transaksi?.length||0) + (data.tagihan?.length||0) + (data.piutang?.length||0) + (data.hutang?.length||0) + (data.aset?.length||0);
  const totalTagihan = TAGIHAN_OPTIONS.length + katTagihanUser.length;
  const totalBudget  = TAGIHAN_OPTIONS.length + katBudgetUser.length;
  ...
  return (
    <div className="fade-in" style={{ width: '100%', paddingBottom: 24 }}>
      ...
      {/* ── Profile Card ─────────────────────────── */}
      <div style={{ background: 'var(--card)', borderRadius: 16, border: '1px solid var(--border)', boxShadow: 'var(--shadow)', marginBottom: 20 }}>
        <div style={{ height: 4, borderTopLeftRadius: 15, borderTopRightRadius: 15, background: 'linear-gradient(90deg, var(--accent2), var(--purple))' }} />
        <div style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, var(--accent2), var(--purple))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', fontWeight: 800, color: '#fff' }}>
            {(user.name || user.username || '?')[0].toUpperCase()}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 800, fontSize: '1.05rem', color: 'var(--text)' }}>{user.name || user.username}</div>
          ...
          <button onClick={() => setPanel('nama')} className="btn btn-ghost btn-sm" style={{ flexShrink: 0 }}>✎ Edit</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', borderTop: '1px solid var(--border)' }}>
          {[
            { label: 'Transaksi', value: data.transaksi?.length||0, color: 'var(--blue)' },
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid var(--border)', boxShadow:'var(--shadow)' }}>
          ...
          <SettingRow icon="💰" color="blue"   label="Sumber Pemasukan"   sub={`${sumber.length} kategori`} onClick={()=>setPanel('sumber')} />
          <SettingRow icon="💸" color="orange" label="Tujuan Pengeluaran" sub={`${tujuan.length} kategori`} onClick={()=>setPanel('tujuan')} />
          <SettingRow icon="🧾" color="yellow" label="Kategori Tagihan"   sub={`${totalTagihan} kategori`} onClick={()=>setPanel('kattagihan')} />
          <SettingRow icon="🎯" color="purple" label="Kategori Budget"    sub={`${totalBudget} kategori`} onClick={()=>setPanel('katbudget')} last />
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize:'0.68rem', fontWeight:800, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'1.2px', marginBottom:8, paddingLeft:4 }}>Data & Backup</div>
        <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid var(--border)', boxShadow:'var(--shadow)' }}>
          <SettingRow icon="📦" color="blue" label="Export Data" sub={`${totalRecords} total record tersimpan`} onClick={()=>setPanel('data')} first last />
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ background:'var(--card)', borderRadius:16, border:'1px solid var(--border)', boxShadow:'var(--shadow)' }}>
          <SettingRow icon="🚀" color="blue"   label="Versi Aplikasi" right={<span style={{ fontFamily:'var(--mono)', fontWeight:700, fontSize:'0.82rem', color:'var(--text3)' }}>1.0.0</span>} first />
          <SettingRow icon="⚛️" color="purple" label="Framework"      right={<span style={{ fontFamily:'var(--mono)', fontWeight:700, fontSize:'0.82rem', color:'var(--text3)' }}>React + Supabase</span>} />
          <SettingRow icon="💾" color="teal"   label="Penyimpanan"    right={<span style={{ fontFamily:'var(--mono)', fontWeight:700, fontSize:'0.82rem', color:'var(--text3)' }}>Supabase Cloud</span>} />
          <div style={{ height: 1, background: 'var(--border)', margin: '0 18px' }} />
          <div style={{ padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, borderBottomLeftRadius: 15, borderBottomRightRadius: 15 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg, var(--accent2), var(--purple))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', fontWeight: 800, color: '#fff' }}>A</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--text)' }}>Ardian Wahyu Saputra, S.Kom</div>
              <div style={{ fontSize: '0.72rem', color: 'var(--text3)', marginTop: 2 }}>Pengembang Aplikasi</div>
            </div>
            <a href="https://www.linkedin.com/in/ardian-wahyu-saputra-s-kom-04ab2b222/" target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 20, background: 'var(--blue-bg)', border: '1px solid #bfdbfe', color: 'var(--accent2)', fontFamily: 'var(--font)', fontWeight: 700, fontSize: '0.78rem', textDecoration: 'none', flexShrink: 0 }}
              onMouseEnter={e => { e.currentTarget.style.background='var(--accent2)'; e.currentTarget.style.color='#fff'; }}
              onMouseLeave={e => { e.currentTarget.style.background='var(--blue-bg)'; e.currentTarget.style.color='var(--accent2)'; }}>
      ...
      <Panel open={panel==='data'} onClose={()=>setPanel(null)} title="Export Data">
        <PanelData data={data} />
      </Panel>
      <Panel open={panel==='sumber'} onClose={()=>setPanel(null)} title="💰 Sumber Pemasukan">
        <PanelKategori title="Sumber" items={sumber} onAdd={addS} onDelete={delS} onEdit={edtS} />
      </Panel>
      <Panel open={panel==='tujuan'} onClose={()=>setPanel(null)} title="💸 Tujuan Pengeluaran">
        <PanelKategori title="Tujuan" items={tujuan} onAdd={addT} onDelete={delT} onEdit={edtT} />
      </Panel>
      <Panel open={panel==='kattagihan'} onClose={()=>setPanel(null)} title="🧾 Kategori Tagihan">
        <PanelKategoriLocked title="Tagihan" defaultItems={TAGIHAN_OPTIONS} userItems={katTagihanUser} onAdd={addTg} onDelete={delTg} onEdit={edtTg} />
      </Panel>
      <Panel open={panel==='katbudget'} onClose={()=>setPanel(null)} title="🎯 Kategori Budget">
        <PanelKategoriLocked title="Budget" defaultItems={TAGIHAN_OPTIONS} userItems={katBudgetUser} onAdd={addBg} onDelete={delBg} onEdit={edtBg} />
      </Panel>
    </div>
  );
}
