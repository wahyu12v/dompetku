    ...
    <div className="card mb-4">
      <div className="section-header">
        <div>
          ...
          <div className="text-muted text-sm" style={{ marginTop: 3 }}>Ganti password akun kamu</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 380 }}>
        <div className="form-group">
          ...
          <input type="password" className="form-input" placeholder="••••••••" value={form.lama} onChange={set('lama')} />
        </div>
        <div className="form-group">
          <label className="form-label">Password Baru</label>
          <input type="password" className="form-input" placeholder="Minimal 4 karakter" value={form.baru} onChange={set('baru')} />
        </div>
        <div className="form-group">
          <label className="form-label">Konfirmasi Password Baru</label>
          <input type="password" className="form-input" placeholder="Ulangi password baru" value={form.konfirmasi} onChange={set('konfirmasi')} />
        </div>
        <button className="btn btn-primary" onClick={handleSave}>Simpan Password</button>
      </div>
    </div>
  );
}
...
function KategoriSection({ title, desc, items, onAdd, onDelete, onEdit }) {
  ...
  const saveEdit = () => {
    ...
    onEdit(editIdx, v); setEditIdx(-1); setEditVal('');
  };
  ...
  return (
    <div className="card mb-4">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <input
          className="form-input" placeholder="Nama kategori baru..."
          value={newVal} onChange={e => setNewVal(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handleAdd()}
          style={{ flex: 1 }}
        />
        <button className="btn btn-primary" onClick={handleAdd} disabled={!newVal.trim()}>+ Tambah</button>
      </div>
            ...
            {editIdx === idx ? (
              <>
                <input
                  ...
                  autoFocus
                />
                <button onClick={saveEdit}  style={{ color: 'var(--green)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: '1rem', padding: '0 2px' }}>✓</button>
                <button onClick={cancelEdit} style={{ color: 'var(--text3)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1rem', padding: '0 2px' }}>✕</button>
              </>
            ) : (
              <>
                <span style={{ fontSize: '0.84rem' }}>{item}</span>
                <button onClick={() => startEdit(idx)} style={{ color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.85rem', padding: '0 2px' }}>✎</button>
                <button onClick={() => onDelete(idx)}  style={{ color: 'var(--text3)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.9rem', padding: '0 2px' }}>✕</button>
              </>
            )}
          </div>
        ))}
...
// ── Main Page ─────────────────────────────────────────────
export default function PengaturanPage({ data, changePassword }) {
  ...
  return (
    <div className="fade-in">
      ...
      <div className="alert warning mb-4">
        ⚠ Menghapus atau mengganti nama kategori tidak mengubah data transaksi yang sudah ada. Hanya mempengaruhi dropdown saat input baru.
      </div>

      <KategoriSection
        title="Kategori Sumber Pemasukan"
        desc="Dipakai di dropdown 'Sumber Pemasukan' saat tambah transaksi"
        items={sumber} onAdd={addSumber} onDelete={deleteSumber} onEdit={editSumber}
      />
      <KategoriSection
        title="Kategori Tujuan Pengeluaran"
        desc="Dipakai di dropdown 'Tujuan Pengeluaran' saat tambah transaksi"
        items={tujuan} onAdd={addTujuan} onDelete={deleteTujuan} onEdit={editTujuan}
      />
    </div>
  );
}
