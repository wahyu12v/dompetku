...
// No.8 — tahun dinamis dari data, bukan hardcoded
...
export default function LaporanPage({ data }) {
  ...
  // No.2 — saldo hanya dari transaksi, bukan tagihan
  const { saldoData, totalPms, totalPgl } = useMemo(()=>{
    let running = saldoAwal, totPms=0, totPgl=0;
    // Hitung saldo running hanya dari transaksi harian (konsisten dengan dashboard)
  ...
  // No.4 — data per kategori untuk chart
  ...
  return (
    <div className="fade-in" style={{ width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 14 }}>
          {/* Year stepper — No.8 dinamis */}
          <div style={{ display: 'flex', alignItems: 'center', background: 'var(--card)', padding: '5px 10px', borderRadius: 30, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', border: '1px solid var(--border)' }}>
            <button onClick={() => { setYear(p => Math.max(minYear, p-1)); setBulan(''); }} disabled={year <= minYear}
              style={{ width:30, height:30, borderRadius:'50%', border:'none', background:'transparent', color:year<=minYear?'var(--border2)':'var(--text)', cursor:year<=minYear?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.9rem' }}>❮</button>
            <div style={{ minWidth:80, textAlign:'center', padding:'0 4px' }}>
              <div style={{ fontSize:'0.6rem', fontWeight:800, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'1px' }}>Tahun</div>
              <div style={{ fontWeight:800, fontSize:'1.05rem', color:'var(--text)', lineHeight:1.2 }}>{year}</div>
            </div>
            <button onClick={() => { setYear(p => Math.min(maxYear, p+1)); setBulan(''); }} disabled={year >= maxYear}
              style={{ width:30, height:30, borderRadius:'50%', border:'none', background:'transparent', color:year>=maxYear?'var(--border2)':'var(--text)', cursor:year>=maxYear?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.9rem' }}>❯</button>
          </div>

          {/* Saldo Awal */}
          <div style={{ display:'flex', alignItems:'center', gap:12, background:'var(--card)', padding:'6px 10px 6px 18px', borderRadius:30, border:'1px solid var(--border)', boxShadow:'0 2px 8px rgba(0,0,0,0.04)', flex:mobile?1:'none' }}>
            <span style={{ fontSize:'0.7rem', fontWeight:800, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'0.5px', whiteSpace:'nowrap' }}>Saldo Awal</span>
            <div style={{ display:'flex', alignItems:'center', background:'var(--bg3)', padding:'5px 12px', borderRadius:20, flex:mobile?1:'none' }}>
              <span style={{ fontWeight:800, color:'var(--text2)', marginRight:5, fontSize:'0.88rem' }}>Rp</span>
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <span style={{ fontSize: '0.72rem', color: 'var(--text3)', fontWeight: 700 }}>Filter:</span>
            <button onClick={() => setFilterKat('')} style={{ padding: '4px 12px', borderRadius: 20, border: `1px solid ${!filterKat ? 'var(--accent2)' : 'var(--border)'}`, background: !filterKat ? 'var(--blue-bg)' : 'transparent', color: !filterKat ? 'var(--accent2)' : 'var(--text3)', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer' }}>Semua</button>
            {allKategori.map(k => (
              <button key={k} onClick={() => setFilterKat(k === filterKat ? '' : k)} style={{ padding: '4px 12px', borderRadius: 20, border: `1px solid ${filterKat === k ? 'var(--accent2)' : 'var(--border)'}`, background: filterKat === k ? 'var(--blue-bg)' : 'transparent', color: filterKat === k ? 'var(--accent2)' : 'var(--text3)', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>{k}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:'flex', overflowX:'auto', gap:8, paddingBottom:4, scrollbarWidth:'none' }}>
          <button onClick={()=>setBulan('')} style={{ flexShrink:0, padding:'7px 18px', borderRadius:24, border:'none', fontFamily:'var(--font)', fontWeight:700, fontSize:'0.83rem', cursor:'pointer', transition:'all 0.18s', whiteSpace:'nowrap', background:bulanDrill===''?'var(--text)':'var(--card)', color:bulanDrill===''?'var(--bg)':'var(--text2)', boxShadow:bulanDrill===''?'0 3px 10px rgba(0,0,0,0.12)':'0 1px 4px rgba(0,0,0,0.04)', border:`1px solid ${bulanDrill===''?'var(--text)':'var(--border)'}` }}>📊 Tahunan</button>
          {Array.from({length:12},(_,i)=>{
            const m = String(i+1).padStart(2,'0');
            const active = bulanDrill === m;
            return (
              <button key={m} onClick={()=>setBulan(m)} style={{ flexShrink:0, padding:'7px 16px', borderRadius:24, fontFamily:'var(--font)', fontWeight:700, fontSize:'0.83rem', cursor:'pointer', transition:'all 0.18s', whiteSpace:'nowrap', background:active?'var(--accent2)':'var(--card)', color:active?'#fff':'var(--text2)', border:`1px solid ${active?'var(--accent2)':'var(--border)'}`, boxShadow:active?'0 3px 10px rgba(3,105,161,0.2)':'0 1px 4px rgba(0,0,0,0.04)' }}>{mobile?monthName(i+1):fullMonth(i+1)}</button>
            );
          })}
        </div>
      </div>
