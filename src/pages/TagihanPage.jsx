    ...
    <div className="confirm-overlay" style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(2px)' }}>
      <div className="confirm-box" style={{ maxWidth: 400, borderRadius: '16px' }}>
        <div style={{ textAlign: 'center', padding: '32px 24px 0' }}>
          <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'var(--green-bg)', color: 'var(--green)', margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>🧾</div>
          ...
          <div style={{ fontSize: '0.85rem', color: 'var(--text2)', lineHeight: 1.6, padding: '0 8px' }}>
            ...
            <span style={{ fontFamily: 'var(--mono)', fontWeight: 800, color: 'var(--orange)' }}>{fmtRp(tagihan.nominal)}</span>
        ...
        <div style={{ display: 'flex', gap: 12, padding: '24px', justifyContent: 'center', marginTop: 8 }}>
          ...
          <button className="btn btn-primary" style={{ flex: 1, borderRadius: '24px', padding: '10px 0', fontWeight: 600 }} onClick={onYes}>Ya, Catat</button>
        </div>
      </div>
    </div>,
    document.body
...
function TagihanCard({ t, onEdit, onDelete, onPaid }) {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '14px 16px', boxShadow: '0 2px 6px rgba(0,0,0,0.02)', borderLeft: `4px solid ${lunas ? 'var(--green)' : overdue ? 'var(--red)' : 'var(--orange)'}`, boxSizing: 'border-box', width: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
        ...
        <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: '0.9rem', color: lunas ? 'var(--text3)' : 'var(--orange)', whiteSpace: 'nowrap', flexShrink: 0, textAlign: 'right' }}>{fmtRp(t.nominal)}</div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '12px', flexWrap: 'wrap' }}>
        ...
        {t.batas && <div style={{ fontSize: '0.72rem', color: overdue ? 'var(--red)' : 'var(--text3)', fontWeight: overdue ? 700 : 500 }}>⏰ {fmtDate(t.batas)}{overdue ? ' ⚠' : ''}</div>}
        <StatusBadge status={t.status} />
      </div>
      <div style={{ display: 'flex', gap: '6px', marginTop: '12px', paddingTop: '10px', borderTop: '1px dashed var(--border)' }}>
        {!lunas && <button className="btn btn-success btn-sm" style={{ flex: 1, borderRadius: '8px', fontSize: '0.75rem', fontWeight: 600, padding: '6px 0' }} onClick={() => onPaid(t)}>✓ Bayar</button>}
...
export default function TagihanPage({ data }) {
  ...
  return (
    <div className="fade-in" style={{ paddingBottom: '30px', width: '100%', boxSizing: 'border-box' }}>
      {ConfirmUI}
      {konfirmasi && <KonfirmasiTransaksi tagihan={konfirmasi} onYes={handleKonfirmasiYes} onNo={() => setKonfirmasi(null)} />}
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', marginBottom: '20px', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--bg3)', padding: '3px', borderRadius: '20px' }}>
            <button onClick={() => setFilterYear(y => Math.max(minYear, y - 1))} disabled={filterYear <= minYear} style={{ width: 26, height: 26, borderRadius: '50%', border: 'none', background: filterYear <= minYear ? 'transparent' : 'var(--card)', color: filterYear <= minYear ? 'var(--border2)' : 'var(--text)', cursor: filterYear <= minYear ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem' }}>❮</button>
            <div style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--text)', minWidth: '40px', textAlign: 'center' }}>{filterYear}</div>
            <button onClick={() => setFilterYear(y => Math.min(maxYear, y + 1))} disabled={filterYear >= maxYear} style={{ width: 26, height: 26, borderRadius: '50%', border: 'none', background: filterYear >= maxYear ? 'transparent' : 'var(--card)', color: filterYear >= maxYear ? 'var(--border2)' : 'var(--text)', cursor: filterYear >= maxYear ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem' }}>❯</button>
          </div>
        </div>
        <div style={{ display: 'flex', overflowX: 'auto', gap: '6px', paddingBottom: '4px', flexWrap: 'nowrap', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none', width: '100%' }}>
          {BULAN_SINGKAT.map((b, i) => {
            ...
            return <button key={numStr} onClick={() => setFilterMonth(numStr)} style={{ flexShrink: 0, background: isActive ? 'var(--blue)' : 'transparent', color: isActive ? '#fff' : 'var(--text2)', border: `1px solid ${isActive ? 'var(--blue)' : 'var(--border)'}`, padding: '6px 14px', borderRadius: '20px', fontWeight: 600, fontSize: '0.78rem', cursor: 'pointer', transition: 'all 0.2s' }}>{b}</button>;
          })}
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden', borderRadius: 12 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border)', gap: '16px' }}>
          <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', scrollbarWidth: 'none' }}>
            {[['semua','Semua'],['belum','Belum Dibayar'],['sudah','Sudah Dibayar']].map(([k, l]) =>
              <button key={k} onClick={() => setTab(k)} style={{ background: tab === k ? 'var(--text)' : 'var(--bg3)', border: 'none', padding: '6px 14px', borderRadius: '20px', cursor: 'pointer', fontWeight: tab === k ? 700 : 600, fontSize: '0.8rem', color: tab === k ? 'var(--bg)' : 'var(--text2)', transition: 'all 0.2s', whiteSpace: 'nowrap', flexShrink: 0 }}>{l}</button>
            )}
          </div>
          <button className="btn btn-primary" style={{ borderRadius: '24px', padding: '8px 16px', fontSize: '0.82rem', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }} onClick={() => setModal({ item: null })}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)', background: 'transparent' }}>
                {['Tanggal','Jenis & Keterangan','Nominal','Batas Bayar','Status','Aksi'].map((h, i) => (
                  <th key={h} style={{ background: 'transparent', padding: '14px 20px', textAlign: i >= 2 && i <= 3 ? 'right' : i === 4 ? 'left' : i === 5 ? 'center' : 'left', fontSize: '0.68rem', fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: '1px solid var(--border)', transition: 'background 0.2s', background: 'transparent' }}>
                    ...
                    <td style={{ padding: '14px 20px', textAlign: 'right', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: '0.88rem', color: lunas ? 'var(--text3)' : 'var(--orange)' }}>{fmtRp(t.nominal)}</td>
                    <td style={{ padding: '14px 20px', textAlign: 'right', fontSize: '0.75rem', color: overdue ? 'var(--red)' : 'var(--text3)', fontWeight: overdue ? 700 : 400, whiteSpace: 'nowrap' }}>{t.batas ? fmtDate(t.batas) : <span style={{ color: 'var(--border2)' }}>—</span>}{overdue && ' ⚠'}</td>
                    <td style={{ padding: '14px 20px' }}><StatusBadge status={t.status} /></td>
                    <td style={{ padding: '14px 20px' }}>
                      <div style={{ display: 'flex', justifyContent: 'center', gap: 6 }}>
                        {!lunas && <button className="btn btn-success btn-sm" style={{ padding: '4px 10px', fontSize: '0.75rem', borderRadius: '6px' }} onClick={() => handlePaid(t)}>✓ Bayar</button>}
                        <button className="btn btn-ghost btn-sm" style={{ padding: '4px 8px' }} onClick={() => setModal({ item: t })}>✎</button>
                        <button className="btn btn-ghost btn-sm" style={{ padding: '4px 8px', color: 'var(--red)' }} onClick={() => handleDelete(t.id)}>✕</button>
                      </div>
                    </td>
                  </tr>
                );
        ...
        <div className="mobile-card-list" style={{ padding: '16px' }}>
          {filtered.length === 0
            ...
            : filtered.map(t => <TagihanCard key={t.id} t={t} onEdit={x => setModal({ item: x })} onDelete={handleDelete} onPaid={handlePaid} />)
          }
        </div>
      </div>
