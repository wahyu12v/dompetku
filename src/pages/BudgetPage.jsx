...
export default function BudgetPage({ data }) {
  ...
  // Semua kategori: tujuan + tagihan bawaan + tagihan user + budget user
  const tujuanList    = kategori?.tujuan  || [];
  const tagihanUser   = kategori?.tagihan || [];
  const budgetUser    = kategori?.budget  || [];
  const allKategori   = [...new Set([...tujuanList, ...TAGIHAN_OPTIONS, ...tagihanUser, ...budgetUser])].sort();
