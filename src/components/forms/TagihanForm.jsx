...
// ============================================================
// TagihanForm.jsx — Form tambah/edit tagihan bulanan
// No.5: jenis tagihan sekarang gabungkan TAGIHAN_OPTIONS + tujuan user
// ============================================================
import { useState } from 'react';
import Modal from '../Modal';
import RupiahInput from '../RupiahInput';
...
export default function TagihanForm({ item, onSave, onClose, tujuanKategori = [] }) {
  const [form, setForm] = useState(item
    ? { ...item, nominal: item.nominal || 0 }
    : DEFAULT
  );

  // Gabungkan opsi bawaan + kategori tujuan user (tanpa duplikat)
  const allOptions = [...new Set([...TAGIHAN_OPTIONS, ...tujuanKategori])];
      ...
      <div className="form-grid">
        ...
        <div className="form-group">
          ...
          <select className="form-select" value={form.alasan} onChange={set('alasan')}>
            ...
            {tujuanKategori.length > 0 && (
              <optgroup label="Kategori Pengeluaranmu">
                {tujuanKategori.filter(o => !TAGIHAN_OPTIONS.includes(o)).map(o => (
                  <option key={o} value={o}>{o}</option>
                ))}
              </optgroup>
            )}
